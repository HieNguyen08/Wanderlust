...
export default function AdminReportsPage({ onNavigate }: AdminReportsPageProps) {
  ...
  const stats = [
    ...
    {
      label: t('admin.bookingsThisMonth'),
      value: "1,245",
      change: "+18.2%",
      trend: "up",
      icon: BookOpen,
    ...
    <AdminLayout currentPage="admin-reports" onNavigate={onNavigate} activePage="admin-reports">
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl text-gray-900 mb-2">{t('admin.reportsAndStatistics')}</h1>
            <p className="text-gray-600">{t('admin.reportsAndStatisticsDesc')}</p>
          </div>
          <div className="flex gap-2">
            <Select defaultValue="this-month">
              ...
              <SelectContent>
                ...
                <SelectItem value="this-week">{t('admin.thisWeek')}</SelectItem>
                <SelectItem value="this-month">{t('admin.thisMonth')}</SelectItem>
                <SelectItem value="last-month">{t('admin.lastMonth')}</SelectItem>
                <SelectItem value="this-year">{t('admin.thisYear')}</SelectItem>
              </SelectContent>
            </Select>
        ...
        <Tabs defaultValue="revenue">
          <TabsList>
            ...
            <TabsTrigger value="bookings">{t('common.bookings')}</TabsTrigger>
            <TabsTrigger value="users">{t('common.users')}</TabsTrigger>
          </TabsList>

          <TabsContent value="revenue" className="mt-6">
            <Card className="p-6">
              <h3 className="text-xl font-semibold text-gray-900 mb-6">{t('admin.revenueChart6Months')}</h3>
                  ...
                  <div key={index} className="flex items-center gap-4">
                    ...
                    <div className="w-32 text-right">
                      <p className="font-semibold text-gray-900">
                        {data.bookings} {t('common.bookings')}
                      </p>
                    </div>
                  </div>
                ))}
              ...
              <div className="mt-8 pt-6 border-t">
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    <p className="text-sm text-gray-600 mb-1">{t('admin.totalRevenue')}</p>
                    <p className="text-2xl font-bold text-gray-900">₫3.81B</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600 mb-1">{t('admin.averagePerMonth')}</p>
                    <p className="text-2xl font-bold text-gray-900">₫635M</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600 mb-1">{t('admin.growth')}</p>
          ...
          <TabsContent value="bookings" className="mt-6">
            <Card className="p-6">
              <h3 className="text-xl font-semibold text-gray-900 mb-6">{t('admin.topMostBookedServices')}</h3>
              <div className="space-y-4">
                {topServices.map((service, index) => (
                  <div key={index} className="flex items-center gap-4 p-4 bg-gray-50 rounded-lg">
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-xs text-gray-600">{t('common.bookings')}</p>
                    </div>
          ...
          <TabsContent value="users" className="mt-6">
            <Card className="p-6">
              <h3 className="text-xl font-semibold text-gray-900 mb-6">{t('admin.userStatistics')}</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <h4 className="font-semibold text-gray-900 mb-4">{t('admin.newUsersByMonth')}</h4>
                ...
                <div>
                  <h4 className="font-semibold text-gray-900 mb-4">{t('admin.overview')}</h4>
                  <div className="space-y-4">
                    ...
                    <div className="p-4 bg-purple-50 rounded-lg">
                      <p className="text-sm text-gray-600 mb-1">{t('admin.returnRate')}</p>
