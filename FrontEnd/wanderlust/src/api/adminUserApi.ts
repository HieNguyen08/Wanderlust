...
export const adminUserApi = {
  getAllUsers: async (): Promise<AdminUser[]> => {
    ...
    return data.map((user: any) => ({
      ...
      email: user.email,
      phone: user.mobile || "",
      role: user.role === "VENDOR" ? "vendor" : user.role ? user.role.toLowerCase() : "user",
  ...
  createUser: async (userData: any) => {
    ...
    const payload = {
      ...
      role: userData.role === "vendor" ? "VENDOR" : userData.role.toUpperCase(),
  ...
  updateUser: async (id: string, userData: any) => {
    ...
    const payload = {
      ...
      email: userData.email,
      mobile: userData.phone,
      role: userData.role ? (userData.role === "vendor" ? "VENDOR" : userData.role.toUpperCase()) : undefined,
      isBlocked: userData.status === "banned",
    };
