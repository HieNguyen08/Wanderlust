...
export function HeroSearchHub({ onNavigate, onSearch }: HeroSearchHubProps) {
  ...
  const [carStartTime, setCarStartTime] = useState("10:00");
  const [carEndDate, setCarEndDate] = useState<Date>();
  const [carEndTime, setCarEndTime] = useState("10:00");
  ...
  const serviceTabs = [
    ...
    { id: "car-rental", label: "Thuê xe", icon: Car },
    { id: "activities", label: "Hoạt động", icon: PartyPopper },
  ] as const;
  ...
  const handleFlightSearch = async () => {
    if (!fromAirport) { toast.error("Vui lòng chọn điểm khởi hành"); return; }
    if (!toAirport) { toast.error("Vui lòng chọn điểm đến"); return; }
    if (fromAirport.code === toAirport.code) { toast.error("Điểm đi và điểm đến không được trùng nhau"); return; }
    if (!flightDepartDate) { toast.error("Vui lòng chọn ngày đi"); return; }
    if (tripType === "round-trip" && !flightReturnDate) { toast.error("Vui lòng chọn ngày về"); return; }
    if (tripType === "round-trip" && flightReturnDate && flightReturnDate < flightDepartDate) { toast.error("Ngày về không được trước ngày đi"); return; }
  ...
  return (
    <div className="w-full max-w-[1200px] mx-auto -mt-24 relative z-20 px-4">
      <SearchLoadingOverlay isLoading={isSearching} searchType="flight" message="Đang tìm kiếm chuyến bay..." />
      <div className="bg-white rounded-3xl shadow-xl p-6 md:p-8">
        ...
        {activeService === "flights" && (
          <div className="space-y-4 relative z-10">
            <div className="flex gap-2">
              <button onClick={() => setTripType("round-trip")} className={cn("px-4 py-2 rounded-lg text-sm transition-all border-2", tripType === "round-trip" ? "bg-blue-100 text-blue-700 border-yellow-400 shadow-md" : "bg-gray-50 text-gray-600 hover:bg-gray-100 border-transparent")}>Khứ hồi</button>
              <button onClick={() => setTripType("one-way")} className={cn("px-4 py-2 rounded-lg text-sm transition-all border-2", tripType === "one-way" ? "bg-blue-100 text-blue-700 border-yellow-400 shadow-md" : "bg-gray-50 text-gray-600 hover:bg-gray-100 border-transparent")}>Một chiều</button>
              <button onClick={() => setTripType("multi-city")} className={cn("px-4 py-2 rounded-lg text-sm transition-all border-2", tripType === "multi-city" ? "bg-blue-100 text-blue-700 border-yellow-400 shadow-md" : "bg-gray-50 text-gray-600 hover:bg-gray-100 border-transparent")}>Nhiều chặng</button>
            </div>
                ...
                <PopoverTrigger asChild>
                  <button className="flex items-center gap-3 p-4 border-2 border-gray-200 rounded-xl hover:border-blue-400 transition-colors text-left">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="font-semibold">{fromAirport ? `${fromAirport.city} (${fromAirport.code})` : "Chọn sân bay"}</div>
                    </div>
                  </button>
                </PopoverTrigger>
                ...
                <PopoverTrigger asChild>
                  <button className="flex items-center gap-3 p-4 border-2 border-gray-200 rounded-xl hover:border-blue-400 transition-colors text-left">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="font-semibold">{toAirport ? `${toAirport.city} (${toAirport.code})` : "Chọn sân bay"}</div>
                    </div>
                  </button>
                </PopoverTrigger>
                <PopoverContent className="w-80 p-0" align="start">
              ...
              <Popover open={openFlightDates} onOpenChange={setOpenFlightDates}>
                <PopoverTrigger asChild>
                  <button className="flex items-center gap-3 p-4 border-2 border-gray-200 rounded-xl hover:border-blue-400 transition-colors text-left">
                    ...
                    <div className="flex-1">
                      <div className="text-xs text-gray-500">{tripType === "one-way" ? "Ngày đi" : "Ngày đi - Ngày về"}</div>
                      <div className="font-semibold">
                        {tripType === "one-way" ? (flightDepartDate ? format(flightDepartDate, "dd/MM/yyyy", { locale: vi }) : "Chọn ngày") : (flightDepartDate && flightReturnDate ? `${format(flightDepartDate, "dd/MM", { locale: vi })} - ${format(flightReturnDate, "dd/MM/yyyy", { locale: vi })}` : flightDepartDate ? format(flightDepartDate, "dd/MM/yyyy", { locale: vi }) : "Chọn ngày")}
                      </div>
                    </div>
                  </button>
                </PopoverTrigger>
                ...
                <PopoverTrigger asChild>
                  <button className="flex items-center gap-3 p-4 border-2 border-gray-200 rounded-xl hover:border-blue-400 transition-colors text-left">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="font-semibold">{adults + children + infants} người · {cabinClass === "economy" ? "Phổ thông" : cabinClass === "business" ? "Thương gia" : "Hạng nhất"}</div>
                    </div>
                  </button>
                </PopoverTrigger>
                <PopoverContent className="w-80" align="start">
                  <div className="space-y-4">
                    <div className="flex items-center justify-between"><span>Người lớn</span><div className="flex items-center gap-3"><Button size="sm" variant="outline" onClick={() => setAdults(Math.max(1, adults - 1))}><Minus className="w-4 h-4" /></Button><span className="w-8 text-center">{adults}</span><Button size="sm" variant="outline" onClick={() => setAdults(adults + 1)}><Plus className="w-4 h-4" /></Button></div></div>
                    <div className="flex items-center justify-between"><span>Trẻ em</span><div className="flex items-center gap-3"><Button size="sm" variant="outline" onClick={() => setChildren(Math.max(0, children - 1))}><Minus className="w-4 h-4" /></Button><span className="w-8 text-center">{children}</span><Button size="sm" variant="outline" onClick={() => setChildren(children + 1)}><Plus className="w-4 h-4" /></Button></div></div>
                    <div className="flex items-center justify-between"><span>Em bé</span><div className="flex items-center gap-3"><Button size="sm" variant="outline" onClick={() => setInfants(Math.max(0, infants - 1))}><Minus className="w-4 h-4" /></Button><span className="w-8 text-center">{infants}</span><Button size="sm" variant="outline" onClick={() => setInfants(infants + 1)}><Plus className="w-4 h-4" /></Button></div></div>
                    <div className="pt-4 border-t"><div className="text-sm mb-2">Hạng vé</div><div className="space-y-2">{[{ value: "economy", label: "Phổ thông" }, { value: "business", label: "Thương gia" }, { value: "first", label: "Hạng nhất" }].map((option) => (<button key={option.value} onClick={() => setCabinClass(option.value as any)} className={cn("w-full text-left px-3 py-2 rounded-lg transition-colors", cabinClass === option.value ? "bg-blue-100 text-blue-700" : "hover:bg-gray-100")}>{option.label}</button>))}</div></div>
                  </div>
                </PopoverContent>
              </Popover>
            </div>
            <Button onClick={handleFlightSearch} className="w-full bg-linear-to-r from-blue-600 to-blue-500 hover:from-blue-700 hover:to-blue-600 h-14 text-lg shadow-lg shadow-blue-500/30 border-2 border-yellow-400"><Search className="w-5 h-5 mr-2" />Tìm chuyến bay</Button>
          </div>
        )}
                ...
                <Popover open={openHotelLocation} onOpenChange={setOpenHotelLocation}>
                  <PopoverTrigger asChild>
                    <button className="flex items-center gap-3 p-4 text-left hover:bg-gray-50 transition-colors">
                      ...
                      <div className="flex-1 min-w-0"><div className="text-xs text-gray-500">Địa điểm</div><div className="font-semibold truncate">{hotelLocation || "Chọn địa điểm"}</div></div>
                        ...
                        <CommandGroup>
                          {[{ code: "HCM", name: "TP. Hồ Chí Minh", count: 450 }, { code: "HN", name: "Hà Nội", count: 380 }, { code: "DN", name: "Đà Nẵng", count: 220 }, { code: "NT", name: "Nha Trang", count: 180 }, { code: "PQ", name: "Phú Quốc", count: 165 }, { code: "HA", name: "Hội An", count: 140 }, { code: "VT", name: "Vũng Tàu", count: 125 }, { code: "DL", name: "Đà Lạt", count: 155 }].map((location) => (
                            <CommandItem key={location.code} value={location.name} onSelect={() => { setHotelLocation(location.name); setOpenHotelLocation(false); }}>
                              ...
                              <div className="flex items-center justify-between w-full"><div className="flex items-center gap-2"><MapPin className="w-4 h-4 text-gray-400" /><span>{location.name}</span></div><span className="text-xs text-gray-500">{location.count} khách sạn</span></div>
                            </CommandItem>
                          ))}
                        </CommandGroup>
                      </CommandList>
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <button className="flex items-center gap-3 p-4 text-left hover:bg-gray-50 transition-colors">
                      ...
                      <div className="flex-1 min-w-0"><div className="text-xs text-gray-500">Nhận phòng - Trả phòng</div><div className="font-semibold truncate">{hotelCheckIn && hotelCheckOut ? `${format(hotelCheckIn, "dd/MM")} - ${format(hotelCheckOut, "dd/MM")}` : "Chọn ngày"}</div></div>
                    </button>
                  </PopoverTrigger>
                ...
                <Popover open={openHotelGuests} onOpenChange={setOpenHotelGuests}>
                  <PopoverTrigger asChild>
                    <button className="flex items-center gap-3 p-4 text-left hover:bg-gray-50 transition-colors">
                      ...
                      <div className="flex-1 min-w-0"><div className="text-xs text-gray-500">Khách & Phòng</div><div className="font-semibold truncate">{hotelGuests + hotelChildren} người · {hotelRooms} phòng</div></div>
                    </button>
                  </PopoverTrigger>
                  <PopoverContent className="w-80" align="start">
                    <div className="space-y-4">
        ...
        {activeService === "car-rental" && (
          <div className="space-y-4 relative z-10">
            <div className="flex gap-2">
              <button onClick={() => setCarDriverType("without")} className={cn("px-4 py-2 rounded-lg text-sm transition-all border-2", carDriverType === "without" ? "bg-blue-100 text-blue-700 border-yellow-400 shadow-md" : "bg-gray-50 text-gray-600 hover:bg-gray-100 border-transparent")}>Tự lái</button>
              <button onClick={() => setCarDriverType("with")} className={cn("px-4 py-2 rounded-lg text-sm transition-all border-2", carDriverType === "with" ? "bg-blue-100 text-blue-700 border-yellow-400 shadow-md" : "bg-gray-50 text-gray-600 hover:bg-gray-100 border-transparent")}>Có tài xế</button>
            </div>
            <div className="grid md:grid-cols-[2fr_1fr_1fr_auto] gap-2">
              <div className="border-2 border-gray-200 rounded-xl p-4 hover:border-blue-400 transition-colors">
                <div className="text-xs text-gray-500">Địa điểm nhận xe</div>
                <Input value={carLocation} onChange={(e) => setCarLocation(e.target.value)} placeholder="Nhập địa điểm..." className="border-none p-0 h-6 focus-visible:ring-0 font-semibold" />
              </div>
              <Popover>
                <PopoverTrigger asChild><button className="w-full p-4 border-2 border-gray-200 rounded-xl text-left hover:border-blue-400"><div className="text-xs text-gray-500">Ngày nhận</div><div className="font-semibold">{carStartDate ? format(carStartDate, "dd/MM/yyyy") : "Chọn ngày"}</div></button></PopoverTrigger>
                <PopoverContent className="w-auto p-0"><Calendar mode="single" selected={carStartDate} onSelect={setCarStartDate} /></PopoverContent>
              </Popover>
              <Popover>
                <PopoverTrigger asChild><button className="w-full p-4 border-2 border-gray-200 rounded-xl text-left hover:border-blue-400"><div className="text-xs text-gray-500">Ngày trả</div><div className="font-semibold">{carEndDate ? format(carEndDate, "dd/MM/yyyy") : "Chọn ngày"}</div></button></PopoverTrigger>
          ...
          <div className="space-y-4 relative z-10">
            <div className="grid md:grid-cols-[2fr_1fr_1fr_auto] gap-2">
              <div className="border-2 border-gray-200 rounded-xl p-4 hover:border-blue-400 transition-colors">
                ...
                <Input value={activityLocation} onChange={(e) => setActivityLocation(e.target.value)} placeholder="Nhập địa điểm..." className="border-none p-0 h-6 focus-visible:ring-0 font-semibold" />
              </div>
              <Popover>
                <PopoverTrigger asChild><button className="w-full p-4 border-2 border-gray-200 rounded-xl text-left hover:border-blue-400"><div className="text-xs text-gray-500">Ngày</div><div className="font-semibold">{activityDate ? format(activityDate, "dd/MM/yyyy") : "Chọn ngày"}</div></button></PopoverTrigger>
              ...
              <Popover open={openActivityGuests} onOpenChange={setOpenActivityGuests}>
                <PopoverTrigger asChild><button className="w-full p-4 border-2 border-gray-200 rounded-xl text-left hover:border-blue-400"><div className="text-xs text-gray-500">Số người</div><div className="font-semibold">{activityAdults + activityChildren} người</div></button></PopoverTrigger>
                <PopoverContent className="w-80"><div className="space-y-4">
                  <div className="flex justify-between items-center"><span>Người lớn</span><div className="flex gap-2"><Button size="sm" variant="outline" onClick={() => setActivityAdults(Math.max(1, activityAdults - 1))}><Minus className="w-4 h-4" /></Button><span>{activityAdults}</span><Button size="sm" variant="outline" onClick={() => setActivityAdults(activityAdults + 1)}><Plus className="w-4 h-4" /></Button></div></div>
                  <div className="flex justify-between items-center"><span>Trẻ em</span><div className="flex gap-2"><Button size="sm" variant="outline" onClick={() => setActivityChildren(Math.max(0, activityChildren - 1))}><Minus className="w-4 h-4" /></Button><span>{activityChildren}</span><Button size="sm" variant="outline" onClick={() => setActivityChildren(activityChildren + 1)}><Plus className="w-4 h-4" /></Button></div></div>
                </div></PopoverContent>
              </Popover>
